...
def generate_user(num_entities: int, batch_size: int):
    ...
    with open('user_data.sql', 'w') as f:
        f.write(f"ALTER TABLE brainbox_user DISABLE TRIGGER ALL;")
        f.write(f"DELETE FROM brainbox_user;")
        f.write(f"DELETE FROM brainbox_registrationcode;")
        f.write(f"ALTER SEQUENCE brainbox_user_id_seq RESTART WITH 1;")
        for index, user in enumerate(zip(usernames, emails, created_dates, updated_dates)):
            ...
            if index % batch_size == 0:
                f.write(f"INSERT INTO brainbox_user (username, email, password, role, created_at, updated_at, last_login, is_superuser, is_staff, is_active) VALUES\n")
            f.write(f"('{username}', '{email}', '{password}', '{role}', '{created_at}', '{updated_at}', null, FALSE, FALSE, TRUE){';' if (index + 1) % batch_size == 0 else ','}\n")
        f.write(f"ALTER TABLE brainbox_user ENABLE TRIGGER ALL;")
...
def generate_profile(num_entities: int, batch_size: int):
    ...
    with open('userprofile_data.sql', 'w') as f:
        f.write(f"ALTER TABLE brainbox_userprofile DISABLE TRIGGER ALL;")
        f.write(f"DELETE FROM brainbox_userprofile;")
        f.write(f"ALTER SEQUENCE brainbox_userprofile_id_seq RESTART WITH 1;")
        for index in range(num_entities):
            bio, website = fake.paragraph(), fake.url()
        ...
        f.write(f"ALTER TABLE brainbox_userprofile ENABLE TRIGGER ALL;")
...
def generate_folder(num_entities: int, num_users: int, batch_size: int):
    ...
    with open('folder_data.sql', 'w') as f:
        f.write(f"ALTER TABLE brainbox_folder DISABLE TRIGGER ALL;")
        f.write(f"DELETE FROM brainbox_folder;")
        f.write(f"ALTER SEQUENCE brainbox_userprofile_id_seq RESTART WITH 1;")
        for index, folder in enumerate(zip(names, created_dates, updated_dates)):
            name, created_at, updated_at = folder
            user_id = index % num_users
            if index % batch_size == 0:
                f.write(f"INSERT INTO brainbox_folder (name, user_id, parent_folder_id, created_at, updated_at) VALUES\n")
            f.write(f"('{name}', {user_id + 1}, null, '{created_at}', '{updated_at}'){';' if (index + 1) % batch_size == 0 else ','}\n")
        f.write(f"ALTER TABLE brainbox_folder ENABLE TRIGGER ALL;")

    toc = time.perf_counter()
    print(f"Generated folders in {toc - tic:0.4f} seconds")


def generate_file(num_entities: int, num_users: int, batch_size: int):
    ...
    with open('file_data.sql', 'w') as f:
        ...
        f.write(f"DELETE FROM brainbox_folder;")
        f.write(f"ALTER SEQUENCE brainbox_userprofile_id_seq RESTART WITH 1;")
        for index, file in enumerate(zip(created_dates, updated_dates)):
            ...
            folder = None if prob < 0.4 else index
            if index % batch_size == 0:
...
def generate_shared_file(num_entities: int, num_users: int, num_relations_per_entity: int, batch_size: int):
    print("Generating shared files...")

    tic = time.perf_counter()

    with open('shared_file_data.sql', 'w') as f:
        f.write(f"ALTER TABLE brainbox_sharedfile DISABLE TRIGGER ALL;")
        f.write(f"ALTER TABLE brainbox_sharedfile DROP CONSTRAINT unique_shared_file;")
        f.write(f"DELETE FROM brainbox_sharedfile;")
        f.write(f"ALTER SEQUENCE brainbox_sharedfile_id_seq RESTART WITH 1;")
        index = 0
        for file_id in range(num_entities):
            user_id = random.randint(0, num_users - 10)
            for count in range(num_relations_per_entity):
                if index % batch_size == 0:
                    f.write(f"INSERT INTO brainbox_sharedfile (user_id, file_id, permission) VALUES\n")
                f.write(f"({user_id + count + 1 if index % batch_size != 0 else 0}, {file_id + 1}, '{random.choice(['R', 'RW'])}'){';' if (index + 1) % batch_size == 0 else ','}\n")
                index += 1
        f.write(f"ALTER TABLE brainbox_sharedfile ENABLE TRIGGER ALL;")
        f.write(f"ALTER TABLE brainbox_sharedfile ADD CONSTRAINT unique_shared_file UNIQUE (user_id, file_id);")
...
def main():
    ...
    print('\n')
    generate_folder(num_entities, num_users, batch_size)
    print('\n')
    generate_file(num_entities, num_users, batch_size)
    print('\n')
    generate_shared_file(num_entities, num_users, num_relations_per_entity, batch_size)
...
#             if index % batch_size == 0:
#                 f.write(f"INSERT INTO brainbox_user (username, email, password, role, created_at, updated_at, last_login, is_superuser, is_staff, is_active) VALUES\n")
#             f.write(f"('{username}', '{email}', '{password}', '{role}', '{created_at}', '{updated_at}', null, 0, 0, 1){';' if (index + 1) % batch_size == 0 else ','}\n")
#
#     toc = time.perf_counter()
#     print(f"Generated users in {toc - tic:0.4f} seconds")
#
